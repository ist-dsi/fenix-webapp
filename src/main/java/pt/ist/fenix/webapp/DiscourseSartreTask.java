...
public class DiscourseSartreTask extends CronTask {
    ...
    public void runTask() throws Exception {
        ...
        usersToCreate.stream()
            .filter(user -> user.getPerson().getInstitutionalEmailAddressValue() != null
                    && !user.getPerson().getInstitutionalEmailAddressValue().isEmpty())
            .forEach(user -> {
                Client.createUser(user.getUsername(), user.getPerson().getInstitutionalEmailAddressValue(), user.getDisplayName());
                try {
                    Thread.sleep(1000);
